...
import com.specure.common.utils.hstoreparser.HstoreParser;
import com.specure.common.utils.hstoreparser.annotation.NonComparableField;
...
public class ResultComparer {
        ...
        if ((result1.getOperator() != null || result1.getEvaluate() != null) && result2.getOperator() == null) {
            for (Field f : parser.getAnnotatedFields()) {
                ...
                if (!f.isAnnotationPresent(NonComparableField.class) && f.get(result1) != null && !Collection.class.isAssignableFrom(f.getType())) {
                    ...
                    f.set(result2, (r instanceof TestScriptInterpreter.EvalResult) ? r : String.valueOf(r));
                }
            }
    ...
    private static int runCompare(AbstractResult<?> result, AbstractResult<?> expectedResult) {
        ...
        if (expectedResult.getOperator() != null) {
            ...
            } else if (eval instanceof TestScriptInterpreter.EvalResult) {
                switch (((TestScriptInterpreter.EvalResult) eval).getType()) {
                    case FAILURE:
                        expectedResult.setOnFailure(((TestScriptInterpreter.EvalResult) eval).getResultKey());
                        return RESULT_FAILURE;
                    case SUCCESS:
                        expectedResult.setOnSuccess(((TestScriptInterpreter.EvalResult) eval).getResultKey());
                        return RESULT_SUCCESS;
                    default:
                        expectedResult.setOnSuccess(((TestScriptInterpreter.EvalResult) eval).getResultKey());
                        return ((TestScriptInterpreter.EvalResult) eval).getType().ordinal();
                }
            } else {
