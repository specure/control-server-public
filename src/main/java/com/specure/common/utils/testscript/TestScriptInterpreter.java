 ...
 ******************************************************************************/
package com.specure.common.utils.testscript;
...
public class TestScriptInterpreter {
     ...
     * @return
     */
    public static <T> Object interprete(String command, Hstore hstore, AbstractResult object, boolean useRecursion, ResultOptions resultOptions) {
    ...
    private static Object eval(String[] args, Hstore hstore, AbstractResult<?> object) throws ScriptException {
        try {
            ...
            try {
                ...
                logger.error("Unable to eval QoS");
            }
            ...
            if (result != null) {
                if (jsEngine.getClass().getCanonicalName().equals("jdk.nashorn.api.scripting.NashornScriptEngine")) {
                    logger.debug("Nashorn engine required");
                } else {
            ...
            if (m.find()) {
                ...
                if (args.length > 1) {
                    try {
                        ...
                        if (args.length > 3) {
                            groupingUsed = ("t".equalsIgnoreCase(args[3]) || "true".equalsIgnoreCase(args[3]));
                        }
    ...
    public static Map<String, Object> jsonToMap(final JSONObject json) {
        @SuppressWarnings("unchecked") final Iterator<String> jsonKeys = json.keys();
