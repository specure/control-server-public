...
public class VideoStreamingQualityCalculatorImpl extends BaseQualityCalculator implements QualityCalculator {
    public VideoStreamingQualityCalculatorImpl() {
        ratingConditionMap.put(UserExperienceQuality.EXCELLENT,
                Map.of(
                        PropertyName.DOWNLOAD_METRIC_PARAMETER, new ParameterCondition(MORE_OR_EQUAL, 25000),
    ...
    public UserExperienceQuality calculate(List<UserExperienceParameter> parameters) {
        ...
        videoStreamingMarks.add(getQuality(parameters, PropertyName.DOWNLOAD_METRIC_PARAMETER));
        videoStreamingMarks.add(getQuality(parameters, PropertyName.UPLOAD_METRIC_PARAMETER));
        videoStreamingMarks.add(getQuality(parameters, PropertyName.PING_METRIC_PARAMETER));
