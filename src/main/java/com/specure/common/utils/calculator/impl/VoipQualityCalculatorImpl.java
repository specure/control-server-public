...
public class VoipQualityCalculatorImpl extends BaseQualityCalculator implements QualityCalculator {
    public VoipQualityCalculatorImpl() {
        ...
        ratingConditionMap.put(UserExperienceQuality.POOR,
                Map.of(
                        PropertyName.DOWNLOAD_METRIC_PARAMETER, new ParameterCondition(MORE_OR_EQUAL, 200),
                        PropertyName.UPLOAD_METRIC_PARAMETER, new ParameterCondition(MORE_OR_EQUAL, 200),
                        PropertyName.PING_METRIC_PARAMETER, new ParameterCondition(LESS, 200)
                ));
        ratingConditionMap.put(UserExperienceQuality.BAD,
                Map.of(
                        PropertyName.DOWNLOAD_METRIC_PARAMETER, new ParameterCondition(LESS, 200),
                        PropertyName.UPLOAD_METRIC_PARAMETER, new ParameterCondition(LESS, 200),
    ...
    public UserExperienceQuality calculate(List<UserExperienceParameter> parameters) {
        ...
        voipMarks.add(getQuality(parameters, PropertyName.DOWNLOAD_METRIC_PARAMETER));
        voipMarks.add(getQuality(parameters, PropertyName.UPLOAD_METRIC_PARAMETER));
        voipMarks.add(getQuality(parameters, PropertyName.PING_METRIC_PARAMETER));
